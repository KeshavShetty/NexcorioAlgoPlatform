...
public class G3GammDeltaSqueezeStraddleAlgoThread extends G3BaseClass implements Runnable{
	...
	public G3GammDeltaSqueezeStraddleAlgoThread(Long napAlgoId, String backTestDateStr) {
		super(napAlgoId);
		...
		try {
			...
			do {
				...
				if (ceStraddleOptionName.equals("")) {
					...
					// If index moves 25pts up
					float newCeUpDelta = ceOptionGreeks.getDelta() + 25f*ceOptionGreeks.getGamma();
					float newPeUpDelta = peOptionGreeks.getDelta() + 25f*peOptionGreeks.getGamma();
					
					float straddlePremiumChangeWhenUp = -25f*(newCeUpDelta + newPeUpDelta);
					
					// If index moves 25pts Down
					float newCeDnDelta = ceOptionGreeks.getDelta() + -25f*ceOptionGreeks.getGamma();
					float newPeDnDelta = peOptionGreeks.getDelta() + -25f*peOptionGreeks.getGamma();
					
					float straddlePremiumChangeWhenDn = 25f*(newCeDnDelta + newPeDnDelta);
					
					fileLogTelegramWriter.write( "newCeUpDelta="+newCeUpDelta+" newPeUpDelta="+newPeUpDelta + " straddlePremiumChangeWhenUp="+straddlePremiumChangeWhenUp);
					fileLogTelegramWriter.write( "newCeDnDelta="+newCeDnDelta+" newPeDnDelta="+newPeDnDelta + " straddlePremiumChangeWhenDn="+straddlePremiumChangeWhenDn);
					
					//entryStraddleOptionNames = getStraddleOptionNamesByDeltaOptimised(this.baseDelta, this.optimalHedgeDistance);
//					float totalTimevalueInRunningStraddle = getTotalTimeValue(ceOptionGreeks, peOptionGreeks);
//					float totalTimevalueInNewStraddle = getTotalTimeValue(getOptionGreeks(entryStraddleOptionNames[0]), getOptionGreeks(entryStraddleOptionNames[1]));
...
//					}
					if ( (straddlePremiumChangeWhenUp < -10f || straddlePremiumChangeWhenDn < -10f) ) {
						needRepositioning = true;
					}
				}

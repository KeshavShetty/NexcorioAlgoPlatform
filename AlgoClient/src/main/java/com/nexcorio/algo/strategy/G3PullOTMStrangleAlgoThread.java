...
public class G3PullOTMStrangleAlgoThread extends G3BaseClass implements Runnable {

	private static final Logger log = LogManager.getLogger(G3PriceParityIVBasedAlgoThread.class);
	
	public float baseDelta = 0.5f;
	
	public G3PullOTMStrangleAlgoThread(Long napAlgoId, String backTestDateStr) {
		super(napAlgoId);
	...
	public void run() {
		...
		try {
			...
			do {
				...
				if (!ceStraddleOptionName.equals("")) updateCurrentOrderBuyPrice(ceStraddleOptionName, ceDbId, ceOptionGreeks.getLtp());
				if (!peStraddleOptionName.equals("")) updateCurrentOrderBuyPrice(peStraddleOptionName, peDbId, peOptionGreeks.getLtp());
				...
				if (Math.abs(ceOptionGreeks.getDelta()) > 0.8f || Math.abs(peOptionGreeks.getDelta()) > 0.8f) {
					prepareExit("Overflow");					
				} else {
					if (Math.abs(ceOptionGreeks.getDelta()) < 0.4f) {
						
						// Exit CE and open fresh position with base delta
					...
					if (Math.abs(peOptionGreeks.getDelta()) < 0.4f) {
						
						// Exit PE and open fresh position with base delta
